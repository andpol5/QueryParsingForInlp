...
def getDirectResourceUrl(locationString, database):
	database.setQuery("""SELECT ?Dir WHERE {?Dir rdfs:label \"""" + locationString + """\"@en; a owl:Thing}""")
...
def getRedirectedResourceURL(locationString, database):
	database.setQuery("""SELECT ?Redir WHERE {dbr:""" + locationString + """ dbo:wikiPageRedirects ?Redir}""")
	database.setReturnFormat(JSON)
	results = database.query().convert()
	for result in results["results"]["bindings"]:
...
def disambiguateCount(locationString, database):
...
if len(userInput) > 1:
	...
	if resourceUrl is not None:
		...
		if locationType is not None: 
			...
			disambCount = int(disambiguateCount(locationString, databaseWrapper)) # Check for disambiguates
			if disambCount>0:
